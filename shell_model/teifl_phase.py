...
phase = np.zeros((N,Nt,2), dtype=np.float64)
label = np.empty((N),    dtype=object)
for i in range(N):
  label[i] = r'$k_{%d}$' % (i+offset)
  for itr in range(Nt):
    theta = np.angle(u[i,itr])
    phase[i,itr,0] = np.cos(theta)
    phase[i,itr,1] = np.sin(theta)
...
for tau in range(1, 6):
  result_dir = f"teifl_phase_tau_{tau}"
  file_path  = os.path.join(result_dir, "teifl.npz")
  TEIFL(phase, tau=tau, dt=dt, lag=1, max_m=2, tol=0.1, k=5, n_threads=4, result_dir=result_dir)
