...
//個数のカウントを演算で
double mutual_info2_wrapper(py::array_t<double, py::array::c_style> x_obj, 
                           py::array_t<double, py::array::c_style> y_obj, int k=5){
  py::buffer_info info_x = x_obj.request();
  py::buffer_info info_y = y_obj.request();
  if (info_x.ndim != 2 || info_y.ndim != 2) {
  ...
  return mutual_info2(&x, &y, k, dx, dy, N);}

//epsをpyで計算したものを使用
double mutual_info_noeps_wrapper(py::array_t<double, py::array::c_style> x_obj, 
                           py::array_t<double, py::array::c_style> y_obj, 
                          py::array_t<double, py::array::c_style> eps_obj, int k=5){
  py::buffer_info info_x = x_obj.request();
  py::buffer_info info_y = y_obj.request();
  py::buffer_info info_eps = eps_obj.request();
  if (info_x.ndim != 2 || info_y.ndim != 2) {
    throw std::runtime_error("Input dimension must be 2");
  }
  if (info_x.itemsize != sizeof(double) || info_y.itemsize != sizeof(double)) {
  ...
  double *x   = static_cast<double*>(info_x.ptr);
  double *y   = static_cast<double*>(info_y.ptr);
  double *eps = static_cast<double*>(info_eps.ptr);
...
return mutual_info_noeps(&x, &y, eps, k, dx, dy, N);}
...
PYBIND11_MODULE(shannon_entropy_cpp, m) {
  ...
  m.def("mutual_info2", &mutual_info2_wrapper,
        py::arg("X"), py::arg("Y"), py::arg("k")=3,
        "Compute mutual information of dataset X and Y using Kraskov's estimator type 1");
  m.def("mutual_info_noeps", &mutual_info_noeps_wrapper,
        py::arg("X"), py::arg("Y"), py::arg("eps"), py::arg("k")=3,
        "Compute mutual information of dataset X and Y using Kraskov's estimator type 1");
}
