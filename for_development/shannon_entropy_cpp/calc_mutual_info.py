...
def mutual_info(X, Y, k=5, Thei=10):
  ...
  for i in range(N):
    ...
    dist, _ = tree_XY.query(np.hstack((X[i], Y[i])), k=k, p=2, workers=-1)
    ...
    nX[i] = np.sum(cdist(X[idx], X[i].reshape(1,-1), metric='euclidean') < half_epsilon_XYkNN)
    nY[i] = np.sum(cdist(Y[idx], Y[i].reshape(1,-1), metric='euclidean') < half_epsilon_XYkNN)
    nN[i] = np.sum(idx)
  valid_idx = (nX > 0) & (nY > 0)
